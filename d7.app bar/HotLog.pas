...
//Windows messaging sub system:
  ...
  UM_STARTNEWLOG    = WM_USER + UM_HLSOON_USED + 14;           // writer thread, start a new log file - rbecker
...
type
 ...
 {Types généraux}
  ...
  TConstArray    = Array of System.TVarRec;                // "Variant open array parameters"
...
Begin
  ...
  LogFileTimer.Interval := OneSecond; //set with SetLogFileTimerInterval before calling StartLogging!
  LogFileTimer.OnTimer := OnLogFileTimer;
  (* end rbecker *)
...
procedure THotLog.OnLogFileTimer(Sender : TObject);
// things to do when the log file timer fires - rbecker
var
  SR : TSearchRec;
  SList : TStringList;
Begin
  (* see if the size of the current log file has exceeded our maximum *)
  try
    if FindFirst(hlWriter.hlFileDef.fileName, faAnyfile, SR) = 0 then
      if SR.Size >= hlWriter.hlFileDef.LogFileMaxSize then
        PostThreadMessage(HLWriter.ThreadID, UM_STARTNEWLOG, 0, 0);
  finally
    FindClose(SR);
  end;

  (* now see if there are any old log files we need to delete.  this
     only needs to be done with safe file names as the original code
     to manage generations works just fine for those files *)
  if hlWriter.hlFileDef.UseSafeFilenames then
  begin
    SList := TStringList.Create;
    try
      SList.Sorted := true;
      if FindFirst(hlWriter.hlFileDef.path + '*.log', faAnyfile, SR) = 0 then
      repeat
        SList.Add(SR.Name);
      until FindNext(SR) <> 0;

      if SList.Count > 0 then
        while SList.Count > hlWriter.hlFileDef.SafegdgMax do
        begin
          DeleteFile(hlWriter.hlFileDef.path + SList.Strings[0]);
          SList.Delete(0);
        end;
    finally
      FindClose(SR);
    ...
    case Item.VType of                            // Now handle special cases.
      ...
      vtInterface:                                // Casting ensures proper reference counting.
        begin
          ...
          {$IFDEF VER130}
            Result.VInterface := Item.VInterface; // Modification DELPHI5
          {$ELSE}
...
begin
  case Item.VType of
    ...
    {$IFDEF VER130}
      vtInterface: Item.VInterface := nil;      // Modification DELPHI5
    {$ELSE}
...
var tmpMsg:tagMSG;
    F:TFileStream;
    WrkHandle : Integer;   // Gonzalo
    WrkOpMode : Word;      // Gonzalo
    TempgdgMax : word;     // rbecker

begin
  // --- Gonzalo Code Begins Here ---
  TRY
    WrkOpMode := hlFileDef.OpMode;
    if (WrkOpMode and fmCreate) = fmCreate then
      begin
        WrkHandle := FileCreate(hlFileDef.fileName);
        if WrkHandle < 0 then
          raise EFCreateError.CreateFmt('Cannot create file %s',
                                        [hlFileDef.fileName]);
        FileClose(WrkHandle);
        WrkOpMode := fmOpenReadWrite OR fmShareDenyWrite;
      end;

    WrkHandle := FileOpen(hlFileDef.fileName, WrkOpMode);
    if WrkHandle < 0 then
      raise EFCreateError.CreateFmt('Cannot create file %s',
                                    [hlFileDef.fileName]);
  ...
  // --- Gonzalo Code Ends Here ---
  // These lines replace the old >>>   F := TFileStream.Create(hlFileDef.fileName, hlFileDef.OpMode);
      ...
      Begin
        If PeekMessage(tmpMsg, 0, 0, 0, PM_REMOVE) then
          Case tmpMsg.message of
            ...
            UM_STARTNEWLOG: //rbecker
               Begin
                 if (hlFileDef.UseFileSizeLimit = true) {and (hlFileDef.gdgMax > 1)} then //do we need the 2nd condition?
                 begin
                   ...
                   (* easiest to just duplicate Gonzalo's code *)
                   WrkOpMode := hlFileDef.OpMode;
                   if (WrkOpMode and fmCreate) = fmCreate then
                   begin
                     WrkHandle := FileCreate(hlFileDef.fileName);
                   ...
                   F := TFileStream.Create(hlFileDef.fileName, WrkOpMode);
                 end;
               End;
          End;{Case}
          Sleep(50);
...
begin
  ...
  SendMessage(Self.FfbMemo.Handle, EM_REPLACESEL, 0, Longint(EmptyStr));
end;
...
{$ifdef USE_SLOW_METHOD}
    ...
    if FfbMemoLimit > 0 then
      while Cardinal(FfbMemo.Lines.Count) > FfbMemoLimit do
        FfbMemo.Lines.Delete(0);
    If FDoScroll Then
    Begin
//      ln := SendMessage(self.FfbMemo.Handle,EM_GETLINECOUNT,0,0);
